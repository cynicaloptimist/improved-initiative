...
const getModifiedXp = (
  enemyChallengeRatings: string[],
  playerCount: number
) => {
  const totalXpBase = getTotalXpBase(enemyChallengeRatings);
  if (playerCount > 0) {
    return (
      totalXpBase * getXpMultiplier(enemyChallengeRatings.length, playerCount)
    );
  }
  return totalXpBase;
};
...
const getDifficulty = (totalXp, playerLevels: number[]) => {
  ...
  if (
    playerLevels.some(
      level => typeof level !== "number" || level < 1 || level > 20
    )
  ) {
    return "";
  }
...
export class DifficultyCalculator {
  public static Calculate(
    ...
    const totalXpBase = getTotalXpBase(enemyChallengeRatings);
    const modifiedXp = getModifiedXp(
